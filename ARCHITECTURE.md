...
- **Recording.** Several backends are available: FFV1/MKV lossless and x264-lossless (RGB-lossless libx264rgb) are both CPU-readback (no GPU); H.264 via NVENC (subprocess), in-process VAAPI H.264, and in-process Vulkan H.264/HEVC (zero-copy, fastest) use the GPU. A `LatestTaskSlot` lets newer frames evict older ones so a slow encoder never back-pressures the compositor. Dual recording (primary plus `--secondary-codec`) writes two output paths from the same frame.
